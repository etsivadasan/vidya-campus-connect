...
<<<<<<< HEAD
  ...
  useEffect(() => {
    ...
    if (auth) {
      unsubFirebase = onAuthStateChanged(auth, (user) => {
        if (user) {
          ...
          // Fall back to check simulated user in case of mixed mode or simulator sign-in
          const dummyUser = localStorage.getItem('campus_directory_sim_user');
          if (dummyUser) {
            setAdminUser(JSON.parse(dummyUser));
          } else {
            setAdminUser(null);
          }
        }
    ...
    // 2. Local Simulated / Storage state checker
    const checkState = () => {
      const dummyUser = localStorage.getItem('campus_directory_sim_user');
      if (dummyUser) {
        setAdminUser(JSON.parse(dummyUser));
        ...
        setAdminUser({
          ...
          name: auth.currentUser.displayName || auth.currentUser.email || 'Admin Staff'
        });
      } else {
        setAdminUser(null);
      }
    ...
    // 3. Listen to events for instant reactivity
    const handleEvents = () => checkState();
    window.addEventListener('storage', handleEvents);
    window.addEventListener('campus_directory_auth_change', handleEvents);

    // 4. Polling fallback to super-guarantee update in single-threaded/iframe contexts
    const pollInterval = setInterval(checkState, 1500);

    return () => {
      if (unsubFirebase) unsubFirebase();
      window.removeEventListener('storage', handleEvents);
      window.removeEventListener('campus_directory_auth_change', handleEvents);
...
<<<<<<< HEAD
            {adminUser ? (
              ...
              <button
                disabled
                className="p-1 px-2 border border-slate-800 bg-slate-900/20 text-slate-500 rounded-md text-[9.5px] font-bold flex items-center gap-1 opacity-50 cursor-not-allowed"
                title="Export ZIP is locked. Please login as Admin first."
              >
